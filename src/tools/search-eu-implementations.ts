/**
 * search_eu_implementations — Search EU directives/regulations that have Nepalese implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_nepalese_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as nepalese_statute_count
  ...
  if (input.has_nepalese_implementation) {
    sql += ' HAVING nepalese_statute_count > 0';
  }
  ...
  const withCitations = rows.map(r => ({
    ...r,
    _citation: { canonical_ref: r.eu_document_id, lookup_tool: 'get_nepalese_implementations' },
  }));
