#!/usr/bin/env tsx
/**
 * Drift detection for Nepal Law MCP.
 *
 * Checks if upstream Nepal Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Nepal Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'nepal-law-mcp/1.0 drift-detect' },
      });
