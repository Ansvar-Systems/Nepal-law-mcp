#!/usr/bin/env tsx
/**
 * Database builder for Nepal Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Nepal Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Nepalese statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'NP');
    insertMeta.run('source', 'lawcommission.gov.np');
