#!/usr/bin/env tsx
/**
 * Nepalese Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://lawcommission.gov.np';
const PORTAL_NAME = 'Nepal Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/nepal-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Nepalese Law MCP — Data Freshness Check');
